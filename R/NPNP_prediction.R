...
NPNP_prediction<-function(test_y,test_com,NPNP_training_results,fpr_0){
  ...
  fnr_fpr<-NPNP_training_results$fnr_fpr
  ...
  if(missing(fpr_0)){
    ...
    thres_optim<-thres[which.min(abs(fpr-fpr_0))]
    predict_d<-as.numeric(test_com>=thres_optim)
    method=paste0("FPR=",fpr_0,sep='')
  }
  list(predict_d=predict_d,method=method)
}
